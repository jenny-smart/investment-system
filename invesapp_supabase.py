...
APP_VERSION = "2026-05-20-v20-cnyes-id"
...
def fetch_fund_nav(code: str, pattern: str) -> tuple[float | None, str]:
    """
    基金淨值多來源抓取，優先順序：
    1. 鉅亨網 JSON API（使用 FUND_CNYES_IDS 對照的精確 ID）
    2. MoneyDJ 網頁爬蟲
    3. 舊版鉅亨 API（FUND_ANUE_IDS）
    4. 鉅亨買基金 anue 格式（A45089 等）
    """
    code = normalize_text(code)
    pattern = normalize_text(pattern)

    # 若 code 是 A45089 格式（舊版鉅亨），直接走舊版 API
    if re.fullmatch(r"[A-Z]\d{5}", code.upper()):
        nav, status = fetch_anue_fund_nav(code)
        if nav:
            return nav, status
        # 再試新版鉅亨 API
        cnyes_id = code  # 有時 A45089 也能當 cnyes_id 試試
        nav, status = fetch_cnyes_fund_nav(cnyes_id)
        if nav:
            return nav, status
        return None, status

    # 來源1：鉅亨 JSON API（使用精確 cnyes_id）
    cnyes_id = FUND_CNYES_IDS.get(code, "")
    if cnyes_id:
        nav, status = fetch_cnyes_fund_nav(cnyes_id)
        if nav:
            return nav, f"鉅亨✓({cnyes_id})"

    # 來源2：MoneyDJ 網頁
    if pattern and pattern.lower() != "anue":
        nav, status = fetch_moneydj_nav(code, pattern)
        if nav:
            return nav, f"MoneyDJ✓"

    # 來源3：舊版鉅亨 API（FUND_ANUE_IDS）
    ...
    fail_reasons = []
    if cnyes_id:
        fail_reasons.append(f"鉅亨({cnyes_id})失敗")
    if pattern and pattern.lower() != "anue":
        fail_reasons.append("MoneyDJ失敗")
    return None, "；".join(fail_reasons) or f"無來源:{code}"
    ...
    with c2:
        ...
        if st.button("測試基金淨值（所有來源）", key="test_fund_nav"):
            ...
            if cnyes_id:
                ...
                results[f"鉅亨API({cnyes_id})"] = {"nav": nav1, "status": st1}
            # 來源2: MoneyDJ
            nav2, st2 = fetch_moneydj_nav(fund_code_test, pattern)
            results[f"MoneyDJ({pattern})"] = {"nav": nav2, "status": st2}
            # 最終
