...
APP_VERSION = "2026-05-20-supabase-v20-fund-nav-read-html"
...
FUND_ANUE_IDS = {
    # MoneyDJ acft94 與鉅亨 A45089 是同一檔基金。
    # Streamlit Cloud 有時連 MoneyDJ 失敗，這檔優先改走鉅亨 JSON API。
    "acft94": "A45089",
}
...
def anue_id_for_fund(code: str) -> str:
    code_key = normalize_text(code).lower()
    return FUND_ANUE_IDS.get(code_key, "")
...
def fetch_fund_nav(code: str, pattern: str) -> tuple[float | None, str]:
    """
    基金淨值：
    1. 若輸入鉅亨代碼，例如 A45089，直接走 Anue API。
    2. 若 MoneyDJ 代碼有對應鉅亨代碼，例如 acft94 -> A45089，優先走 Anue。
    3. Anue 失敗時，fallback MoneyDJ。
    4. MoneyDJ 先用 pandas.read_html 找「最新淨值」表格，再用 BeautifulSoup / regex 備援。
    """
    code = normalize_text(code)
    pattern = normalize_text(pattern)

    if not code:
        return None, "無 fund_code"

    if re.fullmatch(r"[A-Z]\d{5}", code.upper()):
        nav, status = fetch_anue_fund_nav(code)
        if nav is not None:
            return nav, f"ok(鉅亨:{code.upper()})"
        if not pattern:
            return None, status

    anue_id = anue_id_for_fund(code)
    if anue_id:
        anue_nav, anue_status = fetch_anue_fund_nav(anue_id)
        if anue_nav is not None:
            return anue_nav, f"ok(鉅亨:{anue_id})"

    if pattern.lower() == "anue":
        return fetch_anue_fund_nav(code)

    if not pattern:
        return None, "無 fund_pattern"
    ...
    try:
        ...
        try:
            ...
            for table in tables:
                ...
                table_text = " ".join(map(str, table.columns)) + " " + " ".join(
                    map(str, table.head(3).to_numpy().ravel())
                )

                if "最新淨值" in table_text or "淨值日期" in table_text or "淨值" in table_text:
                    if table.shape[1] >= 2:
                        nav_val = to_float(table.iloc[0, 1])
                        if nav_val is not None and 0 < nav_val < 10000:
                            return nav_val, "ok(MoneyDJ淨值表)"

                    for val in table.iloc[0].dropna().tolist():
                        nav_candidate = to_float(val)
                        if nav_candidate is not None and 0 < nav_candidate < 10000:
                            if int(nav_candidate) in range(1900, 2101) and float(nav_candidate).is_integer():
                                continue
                            return nav_candidate, "ok(MoneyDJ淨值表)"

            for table in tables:
                if table.empty or table.shape[1] < 2:
                    continue
                for val in table.iloc[:, 1].dropna().tolist():
                    nav_candidate = to_float(val)
                    if nav_candidate is not None and 0 < nav_candidate < 10000:
                        if int(nav_candidate) in range(1900, 2101) and float(nav_candidate).is_integer():
                            continue
        ...
        if decimal_candidates:
            return float(decimal_candidates[0]), "ok(MoneyDJ)"

        page_text = soup.get_text(" ", strip=True).replace(",", "")
        nums = re.findall(r"(?<!\d)(\d+\.\d+)(?!\d)", page_text)
        ...
        if anue_id:
            anue_nav, anue_status = fetch_anue_fund_nav(anue_id)
            if anue_nav is not None:
                return anue_nav, f"ok(鉅亨備援:{anue_id})"
            return None, f"MoneyDJ錯誤且鉅亨失敗:{str(e)[:24]} / {anue_status[:24]}"
        return None, f"MoneyDJ錯誤:{str(e)[:40]}"
