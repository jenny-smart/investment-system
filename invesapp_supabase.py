...
GAS_REQUEST_TIMEOUT_SECONDS = 6
PRICE_REQUEST_TIMEOUT_SECONDS = 8
FX_FALLBACK_URL = "https://open.er-api.com/v6/latest/{currency}"
...
def fetch_twse_realtime_quote(ticker: str) -> dict[str, Any]:
    ...
    try:
        r = requests.get(url, params=params, timeout=PRICE_REQUEST_TIMEOUT_SECONDS, headers=headers)
        r.raise_for_status()
        data = r.json()
...
def fetch_stock_price(ticker: str, asset_type: str = "") -> tuple[float | None, str]:
    ...
    if is_tw:
        ...
        g_price, g_status = fetch_google_finance_price(ticker, TW_STOCK_EXCHANGES.get(ticker))
...
def fetch_cnyes_fund_nav(cnyes_id: str) -> tuple[float | None, str]:
    ...
    for url in urls:
        try:
            r = requests.get(url, timeout=PRICE_REQUEST_TIMEOUT_SECONDS, headers=headers)
            if r.status_code != 200:
                continue
            data = r.json()
...
def fetch_fx_fallback(currency: str) -> tuple[float | None, str]:
    currency = normalize_text(currency, "TWD").upper()
    if currency == "TWD":
        return 1.0, "ok"
    try:
        r = requests.get(
            FX_FALLBACK_URL.format(currency=currency),
        ...
        if data.get("result") != "success":
            return None, f"備援匯率失敗:{currency}"
        twd = to_float((data.get("rates") or {}).get("TWD"))
        if twd and twd > 0:
            return float(twd), "ER-API"
    except Exception as e:
        return None, f"備援匯率錯誤:{str(e)[:40]}"
    return None, f"備援匯率無TWD:{currency}"
...
def fetch_fx(currency: str) -> tuple[float | None, str]:
    ...
    if price is None:
        fallback, fallback_status = fetch_fx_fallback(currency)
        if fallback is not None:
            return fallback, fallback_status
        return None, f"匯率抓取失敗:{currency}; {status}; {fallback_status}"
...
def calculate_cost_and_value(r: pd.Series, latest_price: float | None, fx: float | None) -> dict[str, Any]:
    ...
    dividend_received_original_total = normalize_number(r.get("dividend_received_original_total", 0), 0)
    dividend_received_total = dividend_received_original_total * fx if fx is not None else 0
...
def effective_position_currency(r: pd.Series | dict[str, Any]) -> str:
    name = normalize_text(r.get("name", ""))
    currency = normalize_text(r.get("currency", "TWD")).upper()
    if "南非幣" in name:
        return "ZAR"
    if "美元" in name or "美金" in name or normalize_text(r.get("asset_type")) == "美股":
        return "USD"
    if "人民幣" in name:
        return "CNY"
    if "日圓" in name or "日幣" in name:
        return "JPY"
    return currency


def effective_fund_fields(r: pd.Series | dict[str, Any]) -> tuple[str, str]:
    return infer_fund_fields(
        normalize_text(r.get("name", "")),
        normalize_text(r.get("fund_code", "")),
        normalize_text(r.get("fund_pattern", "")),
    )
...
def fund_dividend_original_totals(df: pd.DataFrame) -> dict[tuple[str, str, str, str], float]:
    totals: dict[tuple[str, str, str, str], float] = {}
    primary_totals: dict[tuple[str, str, str, str], float] = {}
    source = ensure_columns(df)
    for _, row in source.iterrows():
        if normalize_text(row.get("asset_type", "")) != "基金":
            continue
        currency = effective_position_currency(row)
        fund_code, _ = effective_fund_fields(row)
        key = dividend_group_key(row, "基金", fund_code, currency)
        if key is None:
            continue
        original_total = normalize_number(row.get("dividend_received_original_total", 0), 0)
        totals[key] = totals.get(key, 0) + original_total
        if key not in primary_totals:
            primary_totals[key] = original_total
    return {
        key: primary_totals.get(key, 0) if primary_totals.get(key, 0) > 0 else total
...
def enrich(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, r in df.iterrows():
        ...
        if asset_type in {"台股", "美股"}:
            ...
            fund_code, fund_pattern = effective_fund_fields(r)
        ...
        if asset_type == "基金":
            ...
            if is_dividend_primary:
                calc_row["dividend_received_original_total"] = dividend_original_totals.get(
                    group_key,
                    normalize_number(r.get("dividend_received_original_total", 0), 0),
                )
        ...
        if asset_type == "基金":
            ...
            manual_div = normalize_number(r.get("monthly_dividend_per_unit", 0), 0)
            _gas_div = _fetch_gas_div_for_enrich(_fc) if _fc and manual_div <= 0 else None
            div_per_unit = manual_div or _gas_div or 0
            div_source = "手動" if manual_div else "GAS"
        else:
...
def render_online_sheets_tab() -> None:
    st.subheader("📥 Google Sheet 舊表匯入 / 檢查")
    st.caption("這裡只讀取線上總表、檢查科目與匯入舊資料；正式帳戶餘額與新增交易請到「現金流」。")
    ...
    for query_code in candidates:
        try:
            r = requests.get(gas_url, params={"code": query_code},
                             timeout=GAS_REQUEST_TIMEOUT_SECONDS, headers={"User-Agent": "Mozilla/5.0"})
            if r.status_code != 200:
                continue
            data = r.json()
            if data.get("ok"):
...
def auto_dividend_update(positions: pd.DataFrame) -> int:
    ...
    for (fc, plt, cur), rows in fund_groups.items():
        ...
        try:
            ...
            recompute_position_dividend_original_total(fc, plt, cur, pay_date)
...
def _position_dividend_total_lookup(enriched_df: pd.DataFrame) -> dict[tuple[str, str, str], dict[str, float]]:
    ...
    try:
        source_df = ensure_columns(load_positions())
    except Exception:
        source_df = pd.DataFrame()

    if source_df.empty and enriched_df is not None and not enriched_df.empty:
        source_df = ensure_columns(enriched_df)

    if source_df.empty:
        return lookup

    funds = source_df[source_df["asset_type"].astype(str) == "基金"].copy()
    if funds.empty:
        return lookup
...
def _position_dividend_totals_for(
    ...
    fx_value = normalize_number(fx, 0)
    if fx_value <= 0:
        fx_value = normalize_number(totals.get("fx", 0), 0)
    twd = original * fx_value if fx_value > 0 else 0
...
def _dividend_name_key(value: Any) -> str:
    text = normalize_match_name(value)
    for old, new in {
        "Ｕ": "u", "Ｙ": "y", "Ａ": "a", "Ｂ": "b",
        "（": "(", "）": ")", "－": "-", "–": "-", "—": "-",
    }.items():
        text = text.replace(old.lower(), new).replace(old, new)
    return text


def _dividend_business_key(
    fund_code: Any,
    platform: Any,
    currency: Any,
    fund_name: Any,
...
def _dividend_record_key(record: dict[str, Any]) -> tuple[str, str, str, str]:
    return _dividend_business_key(
        record.get("fund_code", ""),
        record.get("platform", ""),
        record.get("currency", ""),
        record.get("fund_name", record.get("name", "")),
        record.get("ex_date", ""),
        record.get("pay_date", ""),
    )
...
def _combined_dividend_records() -> list[dict[str, Any]]:
    ...
    existing_keys = {_dividend_record_key(r) for r in records}
...
def build_actual_dividend_table(enriched_df: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, grp in _fund_dividend_groups(enriched_df):
        ...
        estimate_key = _dividend_business_key(
            fund_code,
            estimate.get("platform", ""),
            estimate.get("currency", ""),
            estimate.get("fund_name", ""),
            estimate.get("ex_date", ""),
            estimate.get("pay_date", ""),
        )
        if estimate_key[0] and estimate_key[3] and estimate_key in existing_keys:
            continue
        if any(
    ...
    df["_dedupe_key"] = df.apply(
        lambda r: _dividend_business_key(
            r.get("_fund_code", ""),
            r.get("_platform", ""),
            r.get("_currency", ""),
            r.get("_fund_name", r.get("基金名稱", "")),
            r.get("_ex_date", ""),
            r.get("_pay_date", ""),
        ),
        axis=1,
    )
    df["_dedupe_paid"] = df["確認入帳"].apply(lambda v: 1 if normalize_bool(v, False) else 0)
    df["_dedupe_source"] = df["_source_table"].map({"fund_dividends": 2, "dividend_log": 1}).fillna(0)
    df = df.sort_values(
        ["_sort_date", "_dedupe_paid", "_dedupe_source"],
        ascending=[False, False, False],
    ).drop_duplicates("_dedupe_key", keep="first").copy()
...
def _paid_dividend_original_amount(record: dict[str, Any]) -> float:
    if not normalize_bool(record.get("is_paid", False), False):
        return 0.0
    units = normalize_number(record.get("units_at_ex", 0), 0)
    per_unit = normalize_number(record.get("actual_div_amount", 0), 0)
    if per_unit <= 0:
        per_unit = normalize_number(record.get("div_amount", 0), 0)
    amount = units * per_unit if units > 0 and per_unit > 0 else 0
    if amount <= 0:
        twd_total = normalize_number(record.get("twd_total", 0), 0)
        fx = normalize_number(record.get("fx_rate", 0), 0)
        if twd_total > 0 and fx > 0:
            amount = twd_total / fx
    return max(0.0, amount)


def set_position_dividend_original_total_by_fund(
    fund_code: str,
    platform: str,
    currency: str,
    ...
    fund_code_key = normalize_text(fund_code).lower()
    platform_key = normalize_text(platform)
    currency_key = normalize_text(currency).upper()
    if not fund_code_key:
        return False
    try:
        rows = supabase_client().table("positions").select(
    ...
    matched = [
        row for row in rows
        if normalize_text(row.get("fund_code", "")).lower() == fund_code_key
    ]
    if not matched:
        return False
    matched = sorted(
    ...
    for row in matched:
        ...
        if rid == primary_id and dividend_pay_date:
            payload["dividend_pay_date"] = dividend_pay_date
        sb.table("positions").update(payload).eq("id", rid).execute()
    return True


def recompute_position_dividend_original_total(
    fund_code: str,
    platform: str,
    currency: str,
    fallback_pay_date: str = "",
) -> bool:
    fund_code_key = normalize_text(fund_code).lower()
    platform_key = normalize_text(platform)
    currency_key = normalize_text(currency).upper()
    ...
    for record in _combined_dividend_records():
        if normalize_text(record.get("fund_code", "")).lower() != fund_code_key:
            continue
        if normalize_text(record.get("platform", "")) != platform_key:
            continue
        if normalize_text(record.get("currency", "")).upper() != currency_key:
            continue
        ...
        pay_dates.append(record.get("pay_date") or record.get("ex_date"))

    latest_date = fallback_pay_date
    parsed_dates = [d for d in (_parse_dividend_date(v) for v in pay_dates) if d is not None]
    if parsed_dates:
        latest_date = max(parsed_dates).strftime("%Y/%m/%d")
    return set_position_dividend_original_total_by_fund(
        fund_code_key,
        platform_key,
        currency_key,
        sum(paid_by_key.values()),
        latest_date,
    )
...
STOCK_DIVIDEND_COLUMNS = [
    "平台", "股票名稱", "股票代碼", "除權息日", "現金發放日", "股票發放日",
    "除權息股數", "每股現金股利", "預估現金股利", "實際現金股利", "確認現金入帳",
    "每股股票股利", "預估股票股數", "實際股票股數", "確認股票入帳", "備註",
]
...
def _format_tw_stock_ticker(code: Any, fallback_ticker: str = "") -> str:
    ...
    if not code_text:
        return ""
    suffix = ".TWO" if f"{code_text}.TWO" in TW_STOCK_EXCHANGES else ".TW"
...
def _stock_bonus_per_share(value: Any) -> float:
    ...
    if number <= 0:
        return 0.0
    return number / 10 if number > 1 else number
...
def fetch_twse_stock_dividend_announcements() -> list[dict[str, Any]]:
    ...
    try:
        response = requests.get(url, timeout=PRICE_REQUEST_TIMEOUT_SECONDS, headers={"User-Agent": "Mozilla/5.0"})
        response.raise_for_status()
        data = response.json()
    except Exception:
        return []
    ...
    for item in data:
        ...
        rows.append({
            ...
            "source": "TWSE",
            "note": f"TWSE 股利分派候選；董事會日 {_parse_stock_api_date(item.get('董事會（擬議）股利分派日', ''))}",
        })
    return rows
...
def update_stock_cash_dividend_total(ticker: str, platform: str, delta_cash: float) -> bool:
    ...
    supabase_client().table("positions").update({
        "dividend_received_original_total": max(0, current_total + delta_cash),
        "dividend_received_total": 0,
...
def add_zero_cost_stock_dividend_position(ticker: str, platform: str, shares: float, ex_date: str, note: str = "") -> bool:
    ...
    payload = normalize_payload({
        ...
        "ticker": normalize_ticker(primary.get("ticker", ticker)) or _format_tw_stock_ticker(ticker),
        "fund_code": "",
        "fund_pattern": "",
        "currency": "TWD",
        ...
        "purchase_ym": parsed_ex.strftime("%Y/%m") if parsed_ex is not None else "",
...
def _normalize_stock_dividend_upload(upload_df: pd.DataFrame, current_positions: pd.DataFrame) -> pd.DataFrame:
    ...
    for idx, raw in upload_df.iterrows():
        ...
        position_info = lookup.get(_plain_stock_code(raw_ticker), {})
        ...
        shares_at_ex = normalize_number(raw.get("除權息股數", 0), 0) or normalize_number(position_info.get("shares", 0), 0)
        cash_per_share = normalize_number(raw.get("每股現金股利", 0), 0)
        stock_per_share = normalize_number(raw.get("每股股票股利", 0), 0)
        actual_cash = normalize_number(raw.get("實際現金股利", 0), 0) or shares_at_ex * cash_per_share
        actual_stock_shares = normalize_number(raw.get("實際股票股數", 0), 0) or _stock_bonus_shares(shares_at_ex, stock_per_share)
        errors = []
        if not ticker:
            errors.append("股票代碼必填")
        if not normalize_text(raw.get("除權息日", "")):
        ...
        rows.append({
            ...
            "股票代碼": ticker,
            "股票名稱": name,
            "除權息日": normalize_text(raw.get("除權息日", "")),
            "現金發放日": normalize_text(raw.get("現金發放日", "")),
            "股票發放日": normalize_text(raw.get("股票發放日", "")),
            "除權息股數": shares_at_ex,
            "每股現金股利": cash_per_share,
            "預估現金股利": shares_at_ex * cash_per_share,
            "實際現金股利": actual_cash,
            "確認現金入帳": normalize_bool(raw.get("確認現金入帳", False), False),
            "每股股票股利": stock_per_share,
            "預估股票股數": _stock_bonus_shares(shares_at_ex, stock_per_share),
            "實際股票股數": actual_stock_shares,
            "確認股票入帳": normalize_bool(raw.get("確認股票入帳", False), False),
            "備註": normalize_text(raw.get("備註", "")) or "批次匯入",
            "_row_number": idx + 2,
        })
    return pd.DataFrame(rows)
...
def apply_stock_dividend_upload(preview_df: pd.DataFrame) -> tuple[int, int, int, list[str]]:
    ...
    for _, row in preview_df.iterrows():
        ...
        if cash_received and actual_cash > 0:
            if update_stock_cash_dividend_total(ticker, platform, actual_cash):
                cash_synced += 1
            else:
                warnings.append(f"第 {int(row.get('_row_number', 0))} 列現金股利已匯入，但找不到台股主列可同步：{ticker}")
        if stock_received and actual_stock_shares > 0:
            if add_zero_cost_stock_dividend_position(ticker, platform, actual_stock_shares, normalize_text(row.get("除權息日", "")), f"股票股利批次匯入：{normalize_text(row.get('股票名稱', ''))}"):
                stock_rows_added += 1
            else:
...
def render_stock_dividend_upload_section(current_positions: pd.DataFrame) -> None:
    with st.expander("📤 批次匯入台股股利補資料（CSV / Excel）", expanded=False):
        ...
        uploaded = st.file_uploader("上傳台股股利補資料 CSV / Excel", type=["csv", "xlsx", "xls"], key="stock_dividend_upload_file")
        if uploaded is None:
            return
        try:
            upload_df = read_uploaded_table(uploaded)
            ...
            if st.button("匯入台股股利補資料", disabled=not confirm or valid_count <= 0, key="apply_stock_dividend_upload"):
                ...
                st.success(f"已匯入 {inserted} 筆；現金股利同步 {cash_synced} 筆；股票股利新增 0 成本持倉 {stock_rows_added} 筆。")
                if warnings:
                    st.warning("\n".join(warnings[:20]))
                st.cache_data.clear()
                st.rerun()
...
def render_stock_dividend_tab(enriched_df: pd.DataFrame | None = None) -> None:
    ...
    edited = st.data_editor(
        ...
        column_config=col_cfg,
        disabled=[col for col in STOCK_DIVIDEND_COLUMNS if col not in {"實際現金股利", "確認現金入帳", "實際股票股數", "確認股票入帳", "備註"}],
    ...
    if st.button("儲存台股股利確認", key="save_stock_dividend_confirm"):
        ...
        for pos, (_, row) in enumerate(edited.iterrows()):
            ...
            estimated_cash_total = normalize_number(row.get("預估現金股利", 0), 0)
            actual_cash_total = normalize_number(row.get("實際現金股利", 0), 0) or estimated_cash_total
            estimated_stock_shares = normalize_number(row.get("預估股票股數", 0), 0)
            actual_stock_shares = normalize_number(row.get("實際股票股數", 0), 0) or estimated_stock_shares
            payload = {
            ...
            if old_cash != new_cash and actual_cash_total > 0:
                ...
                if not update_stock_cash_dividend_total(normalize_text(source.get("_ticker", row.get("股票代碼", ""))), normalize_text(source.get("_platform", row.get("平台", "台股"))), delta):
                    warnings.append(f"{row.get('股票代碼', '')} 現金股利已更新記錄，但找不到台股主列可同步累計。")
            if not old_stock and new_stock and actual_stock_shares > 0:
                if not add_zero_cost_stock_dividend_position(normalize_text(source.get("_ticker", row.get("股票代碼", ""))), normalize_text(source.get("_platform", row.get("平台", "台股"))), actual_stock_shares, normalize_text(row.get("除權息日", "")), f"股票股利入帳：{row.get('股票名稱', '')}"):
...
def render_actual_dividend_table(df: pd.DataFrame, height_cap: int = 520) -> None:
    ...
    def _option_label(idx: int, row: pd.Series) -> str:
        return (
            f"{idx + 1}｜{normalize_text(row.get('平台', ''))}｜"
            f"{normalize_text(row.get('基金名稱', ''))}｜{normalize_text(row.get('發放日期', ''))}｜"
            f"{money(normalize_number(row.get('實際配息原幣', 0), 0), 2)} {normalize_text(row.get('幣別', ''))}"
        )

    pending_options: dict[str, int] = {}
    confirmed_options: dict[str, int] = {}
    for idx, row in df.iterrows():
        label = _option_label(int(idx), row)
        if normalize_bool(row.get("_確認前", row.get("確認入帳", False)), False):
            confirmed_options[label] = int(idx)
        else:
            pending_options[label] = int(idx)

    with st.form("actual_dividend_confirm_form"):
        confirm_labels = st.multiselect("確認入帳", list(pending_options.keys()))
        cancel_labels = st.multiselect("取消確認", list(confirmed_options.keys()))
        submitted = st.form_submit_button("💾 套用確認狀態")
        recompute_submitted = st.form_submit_button("重算已確認累計")

    if submitted or recompute_submitted:
        changes = {pending_options[label]: True for label in confirm_labels}
        changes.update({confirmed_options[label]: False for label in cancel_labels})
        if not changes and not recompute_submitted:
            st.info("沒有選擇需要更新的記錄。")
            return
        sb = supabase_client()
        updated = 0
        ...
        warnings: list[str] = []
        existing_keys = _existing_dividend_record_keys(_combined_dividend_records())
        for idx, new_confirmed in changes.items():
            row = display_df.iloc[idx]
            old_confirmed = normalize_bool(df.iloc[idx].get("_確認前", False), False)
            ...
            fx = normalize_number(df.iloc[idx].get("_fx_rate", 1), 1)
            payload = {
            ...
            if source_table == "dividend_log_candidate":
                ...
                insert_key = _dividend_record_key(insert_payload)
                if insert_key not in existing_keys:
                    sb.table("dividend_log").insert(insert_payload).execute()
                    existing_keys.add(insert_key)
            else:
                if row_id is None or normalize_text(row_id, "") in {"", "nan", "None"}:
                    warnings.append(f"{row.get('基金名稱', '')} 缺少記錄 ID，略過。")
                    continue
                if source_table == "fund_dividends":
                    payload["updated_at"] = "now()"
                sb.table(source_table).update(payload).eq("id", int(float(row_id))).execute()
            affected_groups[(fund_code, platform, currency)] = pay_date or ex_date
            updated += 1
        if recompute_submitted:
            for _, row in df[df["確認入帳"].apply(lambda v: normalize_bool(v, False))].iterrows():
                affected_groups[(
                    normalize_text(row.get("_fund_code", "")),
                    normalize_text(row.get("_platform", row.get("平台", ""))),
                    normalize_text(row.get("_currency", row.get("幣別", ""))),
                )] = normalize_text(row.get("_pay_date", row.get("發放日期", ""))) or normalize_text(row.get("_ex_date", ""))
        for (fund_code, platform, currency), pay_date in affected_groups.items():
            if not recompute_position_dividend_original_total(fund_code, platform, currency, pay_date):
                warnings.append(f"{fund_code} 累計配息重算失敗，請確認持倉主列。")
        if warnings:
            st.warning("\n".join(warnings))
        if updated or recompute_submitted:
            msg = f"已更新 {updated} 筆確認狀態。" if updated else "已重算已確認配息累計。"
            st.success(msg)
            st.cache_data.clear()
            st.rerun()
        else:
...
def render_dividend_log_tab(enriched_df: pd.DataFrame | None = None) -> None:
    ...
    with st.expander("➕ 手動新增配息記錄（補填歷史）"):
        st.caption("用於補填歷史配息；實際配息留 0 時，已發放記錄會用預估配息代入。")
            ...
            if st.form_submit_button("💾 新增"):
                if not m_fund_code or not m_ex_date:
                    ...
                    try:
                        ...
                        if m_is_paid:
                            recompute_position_dividend_original_total(
                                m_fund_code,
                                m_platform,
                                m_currency,
                                m_pay_date or m_ex_date,
                            )
...
create table if not exists stock_dividend_log (
    ...
    platform text default '台股',
    ticker text default '',
    name text default '',
    currency text default 'TWD',
    ex_date text default '',
    cash_pay_date text default '',
    stock_pay_date text default '',
    shares_at_ex numeric default 0,
    cash_dividend_per_share numeric default 0,
    estimated_cash_total numeric default 0,
    actual_cash_total numeric default 0,
    stock_dividend_per_share numeric default 0,
    estimated_stock_shares numeric default 0,
    actual_stock_shares numeric default 0,
    is_cash_received boolean default false,
    is_stock_received boolean default false,
    source text default '',
    note text default '',
    created_at timestamptz default now()
);
alter table stock_dividend_log add column if not exists platform text default '台股';
alter table stock_dividend_log add column if not exists ticker text default '';
alter table stock_dividend_log add column if not exists name text default '';
alter table stock_dividend_log add column if not exists currency text default 'TWD';
alter table stock_dividend_log add column if not exists ex_date text default '';
alter table stock_dividend_log add column if not exists cash_pay_date text default '';
alter table stock_dividend_log add column if not exists stock_pay_date text default '';
alter table stock_dividend_log add column if not exists shares_at_ex numeric default 0;
alter table stock_dividend_log add column if not exists cash_dividend_per_share numeric default 0;
alter table stock_dividend_log add column if not exists estimated_cash_total numeric default 0;
alter table stock_dividend_log add column if not exists actual_cash_total numeric default 0;
alter table stock_dividend_log add column if not exists stock_dividend_per_share numeric default 0;
alter table stock_dividend_log add column if not exists estimated_stock_shares numeric default 0;
alter table stock_dividend_log add column if not exists actual_stock_shares numeric default 0;
alter table stock_dividend_log add column if not exists is_cash_received boolean default false;
alter table stock_dividend_log add column if not exists is_stock_received boolean default false;
alter table stock_dividend_log add column if not exists source text default '';
alter table stock_dividend_log add column if not exists note text default '';
alter table stock_dividend_log add column if not exists created_at timestamptz default now();
...
tabs = st.tabs(["總覽", "台股", "美股", "基富通", "渣打基金", "台新基金", "資料安全", "工具", "📊 歷史市值", "💰 配息記錄", "📈 台股股利", "📒 線上總表", "💵 現金流", "🏦 銀行明細", "🔍 明細查詢", "🤖 AI交易研究"])
...
with tabs[15]:
    from ai_trading_dashboard import render_ai_trading_dashboard
    render_ai_trading_dashboard(enriched)
