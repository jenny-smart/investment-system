...
也就是「執行配息快照 / 認列」整個從總覽刪掉，只保留同步基金配息按鈕。配息快照那顆比較適合放在「💰 配息記錄」tab 裡。
...
上面那行欄位建議改成：
    ...
    action_col2, _ = st.columns([1, 4])
不要再留 action_col1，因為刪掉後它沒用途了。
        ...
        for idx, row in edited.iterrows():
            ...
            if total_amount > 0:
                ...
                update_position_dividend_original_total(
                    normalize_text(df.iloc[idx].get("_fund_code", "")),
                    normalize_text(df.iloc[idx].get("_platform", "")),
                    normalize_text(df.iloc[idx].get("_currency", "")),
                    delta_original,
                )
            updated += 1
        if updated:
...
適用於 22s
還要改 render_channel_overview_cards() 這一段。你現在這裡有兩個問題：

action_col1, action_col2 = st.columns([1, 1]) 已經沒用了。
action_col2, _ = st.columns([1, 4]) 那段縮排錯，而且少了按鈕判斷，會變成自動同步。
把 render_channel_overview_cards() 裡面這段：
...
改成：
    ...
    if sync_col.button("同步基金每單位月配息", key="sync_fund_monthly_dividend"):
        fund_rows = enriched[
後面整段 fund_rows... if not fund_rows.empty... 都要維持在 if sync_col.button(...) 裡面，也就是縮排 8 格。

另外你總覽 tab 現在有兩個匯率區：

render_channel_overview_cards(enriched)
render_fx_overview_cards()
但 render_channel_overview_cards() 裡面已經有「即時匯率」，所以建議刪掉這行：

    render_fx_overview_cards()
剩下歷史市值刪除功能那段目前縮排看起來是對的。下一個如果報錯，很可能會是 Supabase 的 portfolio_snapshots 沒有 id 欄位。
