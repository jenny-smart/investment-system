...
- 新增 `📒 線上總表` tab，直接讀取主帳本的 `2026細帳`、`每月收入`、`資產總覽` 三個 Google Sheet gid；現金表下一步應把目前數字轉成線上資料表與分類，不只是載入 Sheet 畫面。
...
- `💰 配息記錄` tab 現在分成「預估配息」與「實際配息」兩張表：預估配息用今日累積單位數，實際配息用除息日快照單位數，避免除息日後新增投資被算入已除息配息。
- `匯率` 已從主功能列移除；`批次更新`、`修復排序`、`貼上清單修復`、`全部歸零重建`、`抓價測試` 改集中在 `工具` tab 的下拉選單。
- `預估配息表` 欄位改為平台、基金名稱、幣別、目前單位數、除息日期、配息金額、預估配息金額；`配息記錄表` 欄位改為平台、基金名稱、幣別、配息單位數、發放日期、配息金額、實際配息金額、勾選確認、累計配息金額。
- GAS 端點改回會抓 MoneyDJ `funddividend` 與 `wb05` 配息頁的完整版本，並對基金代號大小寫做備援，避免 `SHZX0`、`TLZO3`、`PIZN8`、`PIZM9`、`ANZB6`、`ANZH2` 配息資料漏抓。
